...
import { transportationFormSchema, type TransportationFormSchema } from "../schemas";
import { TRANSPORTATION_QUERY_KEY, AVIA_SERVICE_CARRIER_NAME } from "../constants";
...
export const TransportationForm = forwardRef<TransportationFormHandle, TransportationFormProps>(
  ({ onSuccess, editData }, ref) => {
    ...
    const { data: carriers } = useQuery<LogisticsCarrier[]>({ queryKey: ["/api/logistics/carriers"] });
    const { data: deliveryLocations } = useQuery<LogisticsDeliveryLocation[]>({ queryKey: ["/api/logistics/delivery-locations"] });
    const { data: deliveryCosts } = useQuery<DeliveryCost[]>({ queryKey: ["/api/delivery-costs"] });
    ...
    useEffect(() => {
      if (editData) {
        const purchasePriceCompositeId =
          editData.purchasePriceId && editData.purchasePriceIndex !== undefined && editData.purchasePriceIndex !== null
            ? `${editData.purchasePriceId}-${editData.purchasePriceIndex}`
            : editData.purchasePriceId || "";
        const salePriceCompositeId =
          editData.salePriceId && editData.salePriceIndex !== undefined && editData.salePriceIndex !== null
        ...
        const resetValues: TransportationFormSchema = {
          ...
          quantityLiters: editData.quantityLiters ? parseFloat(editData.quantityLiters) : null,
          density: editData.density ? parseFloat(editData.density) : null,
          quantityKg: editData.quantityKg ? parseFloat(editData.quantityKg) : null,
        ...
        if (editData.inputMode) setInputMode(editData.inputMode as "liters" | "kg");
      }
    }, [editData]);
    ...
    const createMutation = useMutation({
      mutationFn: async (data: TransportationFormSchema & { isDraft?: boolean }) => {
      ...
      onSuccess: () => {
        ...
        toast({ title: "Сделка создана", description: "Перевозка успешно сохранена" });
        onSuccess?.();
      },
    ...
    const updateMutation = useMutation({
      mutationFn: async (data: TransportationFormSchema & { id: string; isDraft?: boolean }) => {
        const { id, isDraft = false, ...rest } = data;
        const payload = buildPayload(rest as TransportationFormSchema, isDraft);
        const res = await apiRequest("PATCH", `/api/transportation/${id}`, payload);
        if (!res.ok) {
          const err = await res.json();
      ...
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: [TRANSPORTATION_QUERY_KEY] });
        toast({ title: "Сделка обновлена", description: "Перевозка успешно обновлена" });
        onSuccess?.();
      },
      onError: (e: Error) => toast({ title: "Ошибка", description: e.message, variant: "destructive" }),
    });

    const onSubmit = async (data: TransportationFormSchema, isDraftSubmit?: boolean) => {
      const isDraft = isDraftSubmit ?? data.isDraft ?? false;

      if (!isDraft) {
        const kgVal = calculatedKg ? parseFloat(calculatedKg) : 0;
        if (kgVal <= 0) {
          toast({ title: "Ошибка", description: "Укажите корректное количество топлива", variant: "destructive" });
          return;
        }
        if (salePrice === null) {
          toast({ title: "Ошибка", description: "Не указана цена продажи", variant: "destructive" });
          return;
        }
      }
                      ...
                      <FormItem>
                        ...
                        <div className="flex gap-1 items-center">
                          ...
                          {hasPermission("directories", "create") && (
                            <Button type="button" size="icon" variant="outline" onClick={() => setAddSupplierOpen(true)} data-testid="button-add-supplier">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <div className="flex gap-1 items-center">
                          ...
                          {hasPermission("directories", "create") && (
                            <Button type="button" size="icon" variant="outline" onClick={() => setAddCustomerOpen(true)} data-testid="button-add-buyer">
                              <Plus className="h-4 w-4" />
                            </Button>
                          )}
                        </div>
              ...
              <CardContent>
                ...
                {watchFormInputMode === "liters" && calculatedKg && (
                  <p className="text-sm text-muted-foreground mt-2">
                    Расчетный вес: <strong>{formatNumber(parseFloat(calculatedKg))} кг</strong>
                  </p>
                )}
              </CardContent>
            </Card>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <div className="flex gap-1 items-center">
                          ...
                          {hasPermission("directories", "create") && (
                            <Button type="button" size="icon" variant="outline" onClick={() => setAddCarrierOpen(true)} data-testid="button-add-carrier">
                              <Plus className="h-4 w-4" />
                            </Button>
                          )}
                        </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      ...
                      {isAviaService && hasPermission("prices", "create") && (
                        <Button type="button" size="icon" variant="ghost" onClick={() => setAddPurchasePriceOpen(true)} data-testid="button-add-purchase-price">
                          <Plus className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                    {isAviaService ? (
                      ...
                      >
                        ...
                        <SelectContent>
                          {purchasePrices.flatMap((p) => formatPriceOption(p)).map((opt) => (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      ...
                      {hasPermission("prices", "create") && (
                        <Button type="button" size="icon" variant="ghost" onClick={() => setAddSalePriceOpen(true)} data-testid="button-add-sale-price">
                          <Plus className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {salePrices.flatMap((p) => formatPriceOption(p)).map((opt) => (
                          <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
