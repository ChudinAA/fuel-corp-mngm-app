...
export function OptTable({ onEdit, onCopy, onDelete, onAdd }: OptTableProps) {
  ...
  useEffect(() => {
    const lsKey = "lastCreatedDeal_opt";
    ...
    const onDealCreated = (e: Event) => {
      ...
      if (detail?.type === "opt") checkAndApply();
    };
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-center text-xs font-semibold p-1 w-[55px]">
                КГ
              </TableHead>
              <TableHead className="text-right text-xs font-semibold p-1 w-[70px]">
                Цена пок.
              </TableHead>
          ...
          <TableBody>
            ...
            {dealsToDisplay.length === 0 ? (
              ...
              (() => {
                const _todayDeals = dealsToDisplay.filter((d: any) => isCreatedToday(d.createdAt));
                const _olderDeals = dealsToDisplay.filter((d: any) => !isCreatedToday(d.createdAt));
                const _rows: any[] = [];
                ...
                const _renderDeal = (deal: any, isToday: boolean) => {
                  if (!isToday) {
                    const _ds = formatDate(deal.dealDate);
                    if (_ds !== _lastDateSep) {
                      _lastDateSep = _ds;
                      _rows.push(
