...
export function registerPaymentCalendarRoutes(app: Express) {
  ...
  app.patch(
    "/api/payment-calendar/:id",
    requireAuth,
    requirePermission("finance", "edit"),
    async (req, res) => {
      try {
        const userId = req.session.userId;
        const updated = await storage.payments.updatePaymentCalendarItem(
          req.params.id,
          {
            ...req.body,
            updatedById: userId,
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const userId = req.session.userId;
        const deleted = await storage.payments.deletePaymentCalendarItem(
          req.params.id,
          userId,
        );
        res.json(deleted);
