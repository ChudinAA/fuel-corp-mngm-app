...
export function registerCashflowRoutes(app: Express) {
  ...
  app.patch("/api/cashflow/:id", requireAuth, requirePermission("finance", "edit"), logAudit, async (req, res) => {
    try {
      const userId = req.user!.id;
      const updated = await storage.cashflow.updateCashflowTransaction(req.params.id, {
        ...req.body,
        updatedById: userId,
      });
      res.json(updated);
  ...
  app.delete("/api/cashflow/:id", requireAuth, requirePermission("finance", "delete"), logAudit, async (req, res) => {
    try {
      const userId = req.user!.id;
      const deleted = await storage.cashflow.deleteCashflowTransaction(req.params.id, userId);
      res.json(deleted);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
