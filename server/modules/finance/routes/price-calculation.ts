...
export function registerPriceCalculationRoutes(app: Express) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const calculations =
          await storage.priceCalculations.getPriceCalculations({
  ...
  app.get(
    "/api/price-calculations/:id",
    requireAuth,
    requirePermission("finance", "view"),
    async (req, res) => {
      try {
        const calculation = await storage.priceCalculations.getPriceCalculation(
          req.params.id,
        );
        if (!calculation) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const calculation =
          await storage.priceCalculations.createPriceCalculation({
            ...req.body,
            createdById: userId,
          });
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const userId = req.session.userId;
        const updated = await storage.priceCalculations.updatePriceCalculation(
          req.params.id,
          {
            ...req.body,
            updatedById: userId,
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const userId = req.session.userId;
        const deleted = await storage.priceCalculations.deletePriceCalculation(
          req.params.id,
          userId,
        );
        res.json(deleted);
